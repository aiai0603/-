...
public class FrmModifyGood extends JDialog implements ActionListener {
	...
	private BeanGoods BeanGoods=null;
	public FrmModifyGood(FrmShop frmShop, String s, boolean b, BeanGoods curgood) {
		super(frmShop, s, b);
		...
		workPane.add(jt);
		jt.setText(curgood.getGood_name());
		workPane.add(labelPrice);
		workPane.add(jt1);
		jt1.setText(Double.toString(curgood.getGood_price()));
		workPane.add(labelVip);
		workPane.add(jt2);
		jt2.setText(Double.toString(curgood.getGood_sale()));
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource()==this.btnCancel)
			...
			try {
				...
				ex.modifygood(BeanGoods,name,p1,p2);
			} catch (Exception e1) {
				// TODO 自动生成的 catch 块
